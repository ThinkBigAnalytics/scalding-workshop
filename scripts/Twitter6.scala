 ...
 * on the splits, including finding unique values.
...
class Twitter6(args : Args) extends Job(args) {
  ...
  val twits = Tsv(args("input"), twitterSchema)
      .read
  ...
  new RichPipe(twits)
      .debug
      .project('language)
      .filter('language) {lang: String => lang != """\N"""}
      .unique('language)
      .write(Tsv(args("uniques")))
      // .groupAll { group => 
      //   val sum = group.sum('language)
      //   printf("There are %d unique languages\n", sum)
      //   sum
      // }
}
